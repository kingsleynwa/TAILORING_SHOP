<!-- Stewart Massie 
   date: 1/26/2021
   title: unsemantic-grid-responsive-tablet
   code version: mobile/desktop version
   type: web development
   Url: http://campusmoodle.rgu.ac.uk/pluginfile.php/5367556/mod_resource/content/0/unsemantic-grid-responsive-tablet.css
-->


<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Home</title>
    <link rel="stylesheet" href="assets/style.css">
    <link rel="stylesheet" href="assets/unsemantic-grid-responsive-tablet.css">
</head>
<body>
    <header>
        <nav>
            <ul>
                <li><a href="GetMeasured.php">Get Measured</a></li>
                <li><a href="Home.php">Home</a></li>
                <li><a href="GALLERY.php">Gallery</a></li>
                <li><a href="ABOUTUS.php">About Us</a></li>
                <li><a href="LOGIN.php">Log In</a></li>
                <li><a href="SignUp.php">Sign Up</a></li>
            </ul>
        </nav>

    </header>
    <main>
        <img src="assets/images/img1.jpeg" width="100" height="100"alt="CSDM" id="logo">
        <br>
        <hr>
        <div class="grid-container1">
            <embed img class="grid-100"  src="assets/images/vid1.mp4" width="100" height="500"alt="CSDM" id="cslogo"/>
            <hr> 
        </div> 
        
        <br>
    
        <div class="grid-container2">
            <img class="grid-25"  src="assets/images/img14.jpg" width="100" height="300"alt="CSDM" id="cslogo">
            <img class="grid-25"  src="assets/images/img13.jpg" width="100" height="300"alt="CSDM" id="cslogo">
            <img class="grid-25"  src="assets/images/img11.jpg" width="100" height="300"alt="CSDM" id="cslogo">
            <img class="grid-25"  src="assets/images/img10.jpg" width="100" height="300"alt="CSDM" id="cslogo">
        </div>
        <div class="grid-container7">
            <p  class="grid-25"> <b>Custom Tailord Chinos</b>
                Lorem donec massa sapien faucibus et molestie.
                Gestas diam in arcu cursus euismod . <br>
                Sapien nec sagittis aliquam malesuada bibendum. 
               Urna neque viverra justo nec ultrices dui sapien eget mi.<br> 
               Faucibus purus in massa tempor nec. 
               Elit ut aliquam purus sit.</p>  

            <p  class="grid-25"><b>Custom Tailored Shirt</b>
                Lorem donec massa sapien faucibus et molestie.
                Gestas diam in arcu cursus euismod . <br>
                    Sapien nec sagittis aliquam malesuada bibendum. 
                   Urna neque viverra justo nec ultrices dui sapien eget mi.<br> 
                   Faucibus purus in massa tempor nec. 
                   Elit ut aliquam purus sit.</p>  

            <p  class="grid-25"><b>Custom Tailored Suits</b> 
                Lorem donec massa sapien faucibus et molestie.
                Gestas diam in arcu cursus euismod . <br>
                Sapien nec sagittis aliquam malesuada bibendum. 
               Urna neque viverra justo nec ultrices dui sapien eget mi.<br> 
               Faucibus purus in massa tempor nec. 
               Elit ut aliquam purus sit.</p>

            <p  class="grid-25"><b>Custom Tailored Accessories</b>
                Lorem donec massa sapien faucibus et molestie.
                Gestas diam in arcu cursus euismod . <br>
                Sapien nec sagittis aliquam malesuada bibendum. 
               Urna neque viverra justo nec ultrices dui sapien eget mi.<br> 
               Faucibus purus in massa tempor nec. 
               Elit ut aliquam purus sit.</p>        
        </div>    
    </main>
    <br><br>
    <hr>
    <footer>
        <div class="grid-container8">
            <p  class="grid-50"><b>Contact Us:</b>

                <br><br>

               +44 07587665524   <br>  +44 07587665524</p>

            <p  class="grid-50"><b>Email:</b>

                <br><br>

               Kingsley20th@gmail.com   <br>  Kingswo257@gmail.com</p>
           
      
        </div>    

        <br><br>
        
        <h4>Connect With Us</h4>
  <nav>
    <ul>
      <li><a href="http://www.facebook.com">
          <img src="assets/images/nw/fk.jpg" width=20 alt="Facebook"></a></li>
      <li><a href="http://www.twitter.com">
          <img src="assets/images/nw/tw.jpg" width=20 alt="Twitter"></a></li>
      <li><a href="http://www.linkedin.com">
          <img src="assets/images/nw/in.jpg" width=20 alt="linkedin"></a></li>
      <li><a href="http://www.google+community.com">
          <img src="assets/images/nw/g+.jpg" width=20 alt="Google Community"></a></li>
      <li><a href="http://www.pinterest.com">
          <img src="assets/images/nw/pt.jpg" width=20 alt="pinterest"></a></li>
      <li><a href="http://www.instagram.com">
          <img src="assets/images/nw/inst.jpg" width=20 alt="instagram"></a></li>
    </ul>
  </nav>
        
    </footer>
</body>


</html>